    ...
    <div class="card">
        <div class="card-header">
            <form action="{{ route('service.create') }}">
                <button style="margin:-5px;" class="btn btn-primary">Add services, dear admin</button>
            </form>
        </div>
                ...
                <tbody>
                    @foreach ($services as $item)
                        <tr>
                            ...
                            <td>
                                <form style="display: inline" id="delete-form-{{ $item->id }}" action="{{ route('service.destroy', $item->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <i  class="fas fa-trash-alt fa-lg text-danger cursor-pointer mr-2" onclick="confirmDelete('{{ $item->id }}')" title="Delete"></i>
                                                            ...
                                                            <!-- Edit Icon -->
                                <a href="{{ route('service.edit', $item->id) }}" title="Edit">
                                    <i class="fas fa-edit fa-lg  text-primary mr-2"></i>
                                </a>

                                <!-- Show Icon -->
                                <a href="{{ route('service.show', $item->id) }}" title="Show">
