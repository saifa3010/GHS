...
<div class="col-12">
    <div class="card">
        <div class="card-header">
            <form action="{{ route('About.create') }}">
                ...
                <tbody>
                    @foreach ($About as $item)
                        <tr>
                            ...
                            <td>
                                <form style="display: inline" id="delete-form-{{ $item->id }}" action="{{ route('About.destroy', $item->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <i  class="fas fa-trash-alt fa-lg text-danger cursor-pointer mr-2" onclick="confirmDelete('{{ $item->id }}')" title="Delete"></i>
                                                            ...
                                                            <!-- Edit Icon -->
                                <a href="{{ route('About.edit', $item->id) }}" title="Edit">
                                    <i class="fas fa-edit fa-lg  text-primary mr-2"></i>
                                </a>

                                <!-- Show Icon -->
                                <a href="{{ route('About.show', $item->id) }}" title="Show">
